if [ -z ${1+x} ]; then
    echo "Please enter your domain (7b7b7b7b.nip.io):"
    read DOMAIN    
else
    DOMAIN=$1
    echo "Generating mosquitto certificates for domain $DOMAIN"
fi
...
openssl req -new -x509 -days 365000 -subj "/C=CA/ST=CA/L=CA/O=CA/OU=CA/CN="$DOMAIN"" -key ca.key -out ca.crt
...
openssl req -subj "/C=SE/ST=SE/L=SE/O=SE/OU=SE/CN="$DOMAIN"" -new -out server.csr -key server.key
